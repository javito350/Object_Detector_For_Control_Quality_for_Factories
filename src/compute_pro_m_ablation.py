"""
Compute PRO-AUC metrics for FAISS Subquantizer (M) ablation study on Cable category
"""
...
OUT_CSV = ROOT / "results" / "cable_pro_auc_m_ablation.csv"
...
def get_top_k_image_paths(category: str, k: int) -> list[Path]:
    """
    Get the top-k scoring images for a category.
    Since we don't have scoring info, we use a consistent ranking based on test dir.
    """
    test_dir = ROOT / "data" / "mvtec" / category / "test"
    images = []
    for defect_dir in sorted(test_dir.iterdir()):
        if defect_dir.is_dir():
            for img_file in sorted(defect_dir.glob("*.png")):
                images.append(img_file)
    # Return first k images (deterministic ordering)
    return images[:k]


def compute_pro_for_m_value(m_value: int, category: str = "cable") -> dict:
    """Compute PRO-AUC for a specific M value."""
    m_subdir = QUAL_DIR / f"m_{m_value}"
    
    # Pattern to match heatmap files for this M value
    pat = re.compile(rf"^{category}_m{m_value}_heatmap_rank(?P<rank>[1-9]\d*)\.npy$")
    ...
    if not rank_map:
        print(f"[WARNING] No .npy files found for M={m_value} in {m_subdir}")
        return {
            "category": category,
            "m_value": m_value,
    ...
    category_root = ROOT / "data" / "mvtec" / category
    ...
    if t_max <= t_min:
        print(f"[WARNING] No variation in predictions for M={m_value}")
        return {
            "category": category,
            "m_value": m_value,
            "pro_auc_fpr0.3": 0.0,
...
def main():
    print("=" * 70)
    print("Computing PRO-AUC for Cable M-Ablation Study")
    print("=" * 70)
    ...
    for m_value in m_values:
        print(f"\nComputing PRO-AUC for M={m_value}...")
        result = compute_pro_for_m_value(m_value, category="cable")
        rows.append(result)
    ...
    print("\n" + "=" * 70)
    print("RESULTS: Cable Category M-Ablation PRO-AUC")
    print("=" * 70)
